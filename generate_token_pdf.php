...
// Fetch token info from mess_tokens
$stmt = $conn->prepare("
    SELECT token_id, roll_number, meal_type, menu AS menu_items, special_fee, token_date, created_at
    FROM mess_tokens
    WHERE token_id = ? AND token_type = 'Special'
");
...
// Use token_date as expiry
$expiry_date = $token['token_date'];
...
// Start PDF (basic PDF using raw PDF commands)
$pdf = "%PDF-1.4\n";
...
$line_gap = 30; // Vertical gap between lines
...
// Student Roll
$y -= $line_gap;
$content .= "BT\n/F1 14 Tf\n100 $y Td\n(Student Roll: ".$token['roll_number'].") Tj\nET\n";
...
// Items
$y -= $line_gap;
$content .= "BT\n/F1 14 Tf\n100 $y Td\n(Items: ".$token['menu_items'].") Tj\nET\n";

// Fee
$y -= $line_gap;
$content .= "BT\n/F1 14 Tf\n100 $y Td\n(Fee: Rs.".number_format($token['special_fee'],2).") Tj\nET\n";

// Expiry (in red) - now using token_date
$y -= $line_gap;
$content .= "1 0 0 rg\n"; // Red color
